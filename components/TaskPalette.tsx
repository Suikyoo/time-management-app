...
interface Props<T>{
  tasks: T[],
  setTarget: (task: T | null) => void;
  targetTask: T | null;
  className?: string;
  onClickAdd: () => void;
}
export default function TaskPalette<T extends TaskTemplate >({tasks, setTarget, targetTask, onClickAdd, className}: Props<T>) {
    ...
    <ThemedView reset className={`${defaultStyle} ${className || ""}`} style={styles.shadow}>
      <ScrollView horizontal>
        <ThemedView className="flex flex-row">
          {
            tasks.map(t => (
              <TouchableOpacity key={t.id.toString()} onPressOut={() => onPress(t)}>
